...
const OrderDetailsWithUserDirectionPage = () => {
    ...
    useEffect(() => {
        const fetchOrderDetails = async () => {
            try {
                const response = await fetch(`${apiUrl}/delivery/get_order_details_with_user_direction/${orderId}`, {
                    method: 'GET',
                    headers: {
                ...
                setIsOrderPreparing(data.orderData.order_status === 'on_the_way');
            } catch (error) {
    ...
    const reachUserLocation = async () => {
        ...
        try {
            const response = await axios.post(
                `${apiUrl}/delivery/reach_user_location`,
                {
              ...
              showAlert({
                icon: "success",
                title: 'Reached User Location',
                text: 'The order has been Reached User Location successfully!',
                showConfirmButton: true,
                timer: 3000,
                timerProgressBar: true,
                confirmButtonText: "OK",
            }).then(() => {
                navigate(`/order-with-user-details/${orderDetails.id}`);
            });
        ...
        <Grid container spacing={3} style={{ padding: '5px' }}>
            <Grid item xs={12}>
                <Paper elevation={3} style={{ padding: '16px' }}>
                        ...
                        <Box display="flex" gap={1}>
                            {isOrderPreparing && userLocation ? (
                                <>
                                    ...
                                    <Button variant="contained" color="warning" onClick={reachUserLocation} startIcon={<PersonPinCircleTwoToneIcon />}>
                                        Reached Location
                                    </Button>
                                </>
                            ) : (
                    ...
                    {/* User Location Details Section - Added after the table */}
                    {userLocation && (
                        <Box mt={4}>
                                    ...
                                    <Grid item xs={12} sm={8}>
                                        <Box display="flex" flexDirection="column" gap={1}>
                                            <Box display="flex" gap={1}>
                                                ...
                                                <Typography variant="body1">{userLocation.location}</Typography>
                                            </Box>
                                            <Box display="flex" gap={1}>
                                                <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
                                                    Building Details:
                                                </Typography>
                                                <Typography variant="body1">{userLocation.building_details}</Typography>
                                            </Box>
                                            <Box display="flex" gap={1}>
                                                <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
                                                    City:
                                                </Typography>
                                                <Typography variant="body1">{userLocation.city}</Typography>
                                            </Box>
                                            <Box display="flex" gap={1}>
                                                <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
                                                    State:
                                                </Typography>
                                                <Typography variant="body1">{userLocation.state}</Typography>
                                            </Box>
                                            <Box display="flex" gap={1}>
                                                <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
                                                    Pin Code:
                                                </Typography>
                                                <Typography variant="body1">{userLocation.pin_code}</Typography>
                                            </Box>
                                        </Box>
                                    </Grid>
                                </Grid>
...
export default OrderDetailsWithUserDirectionPage;
